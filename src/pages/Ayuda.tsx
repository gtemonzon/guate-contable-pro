...
const helpSections: HelpSection[] = [
  ...
  {
    id: "multi-moneda",
    title: "Manejo Multi-Moneda",
    icon: Banknote,
    description: "Captura, conversión y diferencial cambiario para empresas que operan en más de una moneda.",
    isNew: true,
    steps: [
      { title: "Habilitar monedas en la empresa", description: "Vaya a Empresas → editar empresa → pestaña 'Monedas'. Active las monedas adicionales (ej. USD, EUR) además de la moneda funcional (base) configurada en la empresa. La moneda funcional es la que se usa en todos los reportes y libros oficiales." },
      { title: "Registrar tipos de cambio mensuales", description: "En Configuración → Tipos de Cambio, registre la tasa por moneda, año y mes (1 USD = X moneda funcional). El sistema usa esta tasa para convertir automáticamente todos los movimientos del mes. Puede capturar la tasa también de forma inline cuando se registra una transacción y falta el tipo de cambio del mes." },
      { title: "Cuentas monetarias en moneda extranjera", description: "Marque las cuentas que mantienen saldo en moneda extranjera (Caja USD, Banco USD, Clientes USD, Proveedores USD) con la bandera 'is_monetary'. Estas cuentas se incluyen en la revaluación cambiaria y exigen capturar moneda + tipo de cambio en cada línea." },
      { title: "Capturar partidas en moneda extranjera", description: "Al crear una partida, el encabezado permite elegir la moneda y la tasa. Las líneas se capturan en la moneda seleccionada y el sistema muestra el equivalente funcional en tiempo real. Si la empresa solo tiene moneda funcional, este selector se oculta." },
      { title: "Configurar cuentas FX en la empresa", description: "En Configuración → Cuentas Especiales, defina: Ganancia/Pérdida cambiaria realizada (cuando se cobra o paga una factura) y Ganancia/Pérdida cambiaria NO realizada (revaluación de saldos al cierre). Sin estas cuentas no se podrán generar las partidas automáticas del módulo FX." },
      { title: "Diferencial cambiario realizado (al cobrar / pagar)", description: "Al guardar una partida de pago o cobro en moneda extranjera que afecte cuentas de Clientes/Proveedores, el sistema detecta automáticamente la posibilidad de un diferencial y abre el diálogo 'Liquidar factura en moneda extranjera'. Allí selecciona la factura origen, captura el monto liquidado y el sistema calcula y contabiliza la ganancia o pérdida cambiaria realizada con base en la diferencia entre la tasa original y la tasa del pago." },
      { title: "Liquidación manual desde la factura", description: "También puede liquidar manualmente desde el detalle de una compra o venta en moneda extranjera con el botón 'Liquidar diferencial'. Soporta liquidaciones parciales: el saldo abierto se mantiene en tab_fx_open_balances hasta que se cubra el total." },
      { title: "Revaluación cambiaria NO realizada (al cierre del mes)", description: "Desde Partidas → 'Revaluación cambiaria' o desde el Asistente de Cierre, ejecute la revaluación de saldos. El sistema toma todas las cuentas monetarias en moneda extranjera con saldo, las revalúa al tipo de cambio del mes y genera una partida automática (DIFC-NR) con la diferencia contra las cuentas de ganancia/pérdida no realizada. La partida se reversa automáticamente el primer día del mes siguiente." },
      { title: "Gating en el cierre de período", description: "El Asistente de Cierre de Período exige que se haya ejecutado la revaluación cambiaria del último mes antes de avanzar al cierre. Si no existe, muestra una alerta con el botón 'Ejecutar revaluación ahora' y permite saltar el paso solo marcando explícitamente la casilla 'Omitir revaluación cambiaria'." },
      { title: "Reportes con moneda original", description: "El Libro Mayor y el Reporte de Partidas muestran una columna adicional con el monto en la moneda original y la tasa aplicada (ej. 'USD 1,000.00 @ 7.7800') cuando la cuenta o la partida tienen movimientos en moneda extranjera. El total siempre se reporta en moneda funcional." },
      { title: "Historial de revaluaciones", description: "En Partidas → pestaña 'Historial FX' puede consultar todas las corridas de revaluación (UNREALIZED), su estado (POSTED / REVERSED) y reversar manualmente una revaluación si fue ejecutada por error." },
    ],
    tips: [
      "Registre el tipo de cambio del mes ANTES de capturar transacciones del mes para evitar tener que detenerse a capturarlo en cada partida.",
      "La moneda funcional siempre tiene tasa 1 y no es editable.",
      "Si una cuenta de Clientes/Proveedores está en moneda extranjera, márquela como 'is_monetary' para que entre a la revaluación.",
      "El diferencial realizado se sugiere automáticamente al guardar la partida de pago: no es necesario abrir el diálogo manualmente.",
      "La revaluación NO realizada se reversa el primer día del mes siguiente: no afecta el resultado acumulado del año, solo refleja la posición a la fecha de cierre.",
      "Antes de cerrar un período, verifique que todas las facturas en moneda extranjera estén liquidadas o tengan saldo abierto registrado correctamente.",
    ],
  },
  {
    id: "inicio",
  ...
  },
  {
    ...
    subsections: [
      ...
      },
      {
        ...
        isNew: true,
        steps: [
          { title: "Seleccionar Cuenta e Importar Estado", description: "Elija la cuenta bancaria y suba el Excel/CSV del estado de cuenta. Configure el mapeo de columnas (fecha, descripción, débito, crédito)." },
          { title: "Guardar Plantilla de Mapeo", description: "Guarde el mapeo de columnas como plantilla por banco para reutilizarla en futuras importaciones." },
          { title: "Auto-Match (Sugerencias Automáticas)", description: "El sistema cruza montos, fechas y referencias contra las partidas contabilizadas y sugiere coincidencias para aceptar en bloque." },
          { title: "Vincular Manualmente y Conciliar", description: "Cuando no hay match automático, asocie cada movimiento bancario con la partida correspondiente y márquelo como conciliado." },
          { title: "Registrar Ajustes", description: "Capture cargos/abonos del banco no contabilizados o viceversa para explicar diferencias entre saldos." },
        ],
      ...
      {
        id: "conciliacion-cuadratica",
        title: "Conciliación Cuadrática (Formato SAT)",
        description: "Genere el reporte oficial de conciliación bancaria con firma del Contador.",
        route: "/conciliacion",
        isNew: true,
        steps: [
          { title: "Definir Saldos Iniciales", description: "Ingrese saldo inicial del banco y de libros para el período conciliado. El sistema valida que cuadren con la conciliación anterior." },
          { title: "Verificar Totales del Período", description: "Revise totales de ingresos y egresos según banco y libros. Las diferencias se explican con los ajustes registrados." },
          { title: "Movimientos Pendientes (No Conciliados)", description: "El reporte prioriza los movimientos no conciliados con sumatoria final, que debe igualar la diferencia entre saldo contable y bancario." },
          { title: "Firma del Auditor / CPA", description: "Indique nombre y número de colegiado del CPA. Puede configurar el auditor predeterminado en la empresa para no reescribirlo." },
          { title: "Exportar PDF (SAT) o Excel", description: "Exporte el PDF con formato cuadrático listo para SAT, o el Excel con detalle completo para análisis interno." },
        ],
        tips: ["El PDF muestra solo movimientos pendientes para mantener el formato compacto.", "El Excel incluye conciliados y no conciliados para auditoría detallada."],
      },
      {
        id: "nomina",
        title: "Nómina",
        description: "Gestione planillas mensuales y genere la póliza contable.",
        route: "/nomina",
        isNew: true,
        steps: [
          { title: "Ver Períodos en Tarjetas o Tabla", description: "Cambie entre vista de tarjetas y vista de tabla. La tabla soporta búsqueda por mes/año y ordenamiento por período, fecha de pago, estado y montos." },
          { title: "Importar Planilla desde Excel", description: "Suba el Excel con sueldos, bonificaciones, IGSS laboral, ISR e indemnización por colaborador. El sistema valida y resume antes de guardar." },
          { title: "Calcular Provisiones (Guatemala)", description: "El sistema aplica tasas locales: IGSS patronal 12.67%, aguinaldo 8.33%, bono 14 8.33%, indemnización 9.72%. Vacaciones (4.17%) son opcionales mediante un switch." },
