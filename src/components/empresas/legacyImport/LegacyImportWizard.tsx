...
const TABLE_CONFIG = [
  {
    key: "accounts",
    label: "Cuentas",
    sourceCount: (dataset: ParsedDataset) => dataset.accounts.length,
  },
  {
    key: "periods",
    label: "Períodos",
    sourceCount: (dataset: ParsedDataset) =>
      new Set(
  ...
  },
  {
    key: "purchases",
    label: "Compras",
    sourceCount: (dataset: ParsedDataset) => dataset.purchases.length,
  },
  {
    key: "sales",
    label: "Ventas",
    sourceCount: (dataset: ParsedDataset) => dataset.sales.length,
  },
  {
    key: "journalEntries",
    label: "Partidas",
    sourceCount: (dataset: ParsedDataset) => dataset.journalEntries.length,
  },
  {
    key: "assetCategories",
    label: "Categorías de activos",
    sourceCount: (dataset: ParsedDataset) => dataset.assetCategories.length,
  },
  {
    key: "fixedAssets",
    label: "Activos fijos",
    sourceCount: (dataset: ParsedDataset) => dataset.fixedAssets.length,
  },
] as const;
...
export function LegacyImportWizard({
  ...
  const [tableDecisions, setTableDecisions] = useState<
    Record<string, TableDecisionMode>
  >({});
  const [now, setNow] = useState(Date.now());
  const isClearJob =
    !!job &&
    (job.payload?.action === "clear" ||
      !!job.result?.cleared ||
      /^Borrando|^Preparando borrado|^Borrado/i.test(job.current_step ?? ""));
  ...
  const handleResume = async () => {
    ...
    setResuming(true);
    try {
      await supabase.functions.invoke("legacy-import-runner", {
        body: { jobId: job.id },
      });
      toast({
        title: "Reanudado",
  ...
  const tablesNeedingDecision = relevantTableRows.filter(
    (item) => item.existing > 0,
  );
  ...
  const fetchActiveJob = useCallback(async () => {
    const { data } = await supabase
      ...
      .select(
        "id, status, current_step, current_count, total_count, errors, result, error_message, finished_at, updated_at, started_at, payload",
      )
      .eq("enterprise_id", enterpriseId)
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const needsDecision = TABLE_CONFIG.some(
        (config) =>
          config.sourceCount(dataset) > 0 && (stats[config.key] ?? 0) > 0,
      );
      if (needsDecision) {
  ...
  const startImport = async (stats?: Record<string, number>) => {
    ...
    const payloadBlob = new Blob([JSON.stringify(dataset)], {
      type: "application/json",
    });
    const { error: upErr } = await supabase.storage
      .from("legacy-imports")
      .upload(payloadPath, payloadBlob, {
        contentType: "application/json",
        upsert: true,
      });
    if (upErr)
      throw new Error(
        `No se pudo subir el archivo de importación: ${upErr.message}`,
      );
    ...
    const { data: jobData, error: jobErr } = await supabase
      ...
      .insert({
        ...
        payload: {
          importPlan: {
            clearExisting: true,
            decisions: TABLE_CONFIG.filter(
              (config) => config.sourceCount(dataset) > 0,
            ).map((config) => ({
              tableKey: config.key,
              mode:
                (stats?.[config.key] ?? 0) > 0
                  ? (tableDecisions[config.key] ?? "clear_then_import")
                  : "clear_then_import",
            })),
          },
        },
      ...
      .single();
    if (jobErr || !jobData)
      throw jobErr ?? new Error("No se pudo crear el job");
    ...
    toast({
      ...
      description:
        "El proceso continuará en segundo plano. Puedes cerrar esta ventana.",
    });
  };

  const handleClearEnterpriseData = async () => {
    const confirmed = window.confirm(
      `¿Borrar los datos importados de ${enterpriseName} para volver a probar?`,
    );
    if (!confirmed) return;
    ...
    <>
            ...
            {step === 1 && (
              <div className="space-y-4">
                <div className="flex justify-end">
                  <Button
                    variant="destructive"
                    onClick={handleClearEnterpriseData}
                      ...
                      {parsing ? (
                        ...
                        <>
                          ...
                          <p className="text-sm mb-4">
                            Sube el archivo .xlsx exportado del sistema legado
                          </p>
                          <input
                            type="file"
                            accept=".xlsx,.xls"
                ...
                <div className="text-xs text-muted-foreground">
                  ...
                  <ul className="list-disc list-inside space-y-0.5">
                    <li>
                      <code>tbl_cuentas</code> — Catálogo de cuentas
                    </li>
                    <li>
                      <code>tbl_compras</code> — Libro de compras
                    </li>
                    <li>
                      <code>tbl_ventas</code> — Libro de ventas
                    </li>
                    <li>
                      <code>tbl_diario</code> + <code>tbl_diario_Detalle</code>{" "}
                      — Partidas
                    </li>
                    <li>
                      <code>tbl_grupoActivos</code> +{" "}
                      <code>tbl_ActivosFijo</code> — Activos fijos
                    </li>
                  </ul>
                </div>
              </div>
              ...
              <div className="space-y-4">
                  ...
                  return (
                    <div className="grid grid-cols-3 gap-2 text-center text-sm">
                      ...
                      <Card>
                        <CardContent className="p-3">
                          <div className="text-2xl font-bold">
                            {dataset.assetCategories.length}
                          </div>
                          <div className="text-xs text-muted-foreground">
                            Grupos activos
                          </div>
                ...
                <Card>
                  <CardContent className="p-4 text-sm space-y-1">
                    <div>
                      📒 Catálogo:{" "}
                      <strong>{dataset.accounts.length} cuentas</strong>.
                    </div>
                    <div>
                      🧾 Diario: solo líneas con{" "}
                      <strong>mostrar = Verdadero</strong>.
                    </div>
                    <div>
                      🏬 Sucursales:{" "}
                      <strong>
                  ...
                  <div className="p-3 space-y-3 text-xs">
                    <div>
                      <p className="font-medium mb-1">
                        Compras (primeros 5 de {dataset.purchases.length}):
                      </p>
                    ...
                    <div>
                      <p className="font-medium mb-1">
                        Ventas (primeros 5 de {dataset.sales.length}):
                      </p>
                      {dataset.sales.slice(0, 5).map((s, i) => (
                        <div key={i} className="flex gap-2 border-b py-1">
                          ...
                          <span className="flex-1 truncate">
                            {s.customerName}
                            {s.branchCode ? ` [Suc.${s.branchCode}]` : ""}
                          </span>
                          <span className="font-mono">
                            Q{s.totalAmount.toFixed(2)}
                          </span>
                        </div>
                      ))}
                    </div>
                          ...
                          <div key={i} className="border rounded p-2 mb-1">
                            <div className="flex justify-between mb-1">
                              <span className="font-medium">
                                {e.date} · {e.description.slice(0, 60)}
                              </span>
                              <Badge
            ...
            {step === 4 && (
              <div className="space-y-4">
                ...
                {isRunning &&
                  job &&
                  (() => {
                    const updatedAtMs = job.updated_at
                      ? new Date(job.updated_at).getTime()
                      : 0;
                    const secondsSinceUpdate = updatedAtMs
                      ? Math.max(0, Math.floor((now - updatedAtMs) / 1000))
                      : null;
                    const stalled =
                      secondsSinceUpdate !== null && secondsSinceUpdate > 90;
                    return (
                      <>
                        <div className="flex items-start gap-2 p-3 rounded bg-muted text-xs">
                          <Info className="h-4 w-4 shrink-0 mt-0.5" />
                          <div>
                            Este proceso corre en{" "}
                            <strong>segundo plano en el servidor</strong>.
                            Puedes cerrar esta ventana o el navegador — incluso
                            seguir desde otro equipo o tu celular abriendo
                            nuevamente "Importar datos legado" en esta empresa.
                          </div>
                        </div>
                        <div className="space-y-2">
                          ...
                          <Progress
                            value={
                              job.total_count > 0
                                ? (job.current_count / job.total_count) * 100
                                : 0
                            }
                          />
                          ...
                          {secondsSinceUpdate !== null && (
                            <p
                              className={`text-xs text-center ${stalled ? "text-destructive font-medium" : "text-muted-foreground"}`}
                            >
                              {stalled ? (
                                <>
                                  ⚠ Sin actividad hace {secondsSinceUpdate}s —
                                  el proceso podría haberse detenido. Pulsa
                                  "Reanudar".
                                </>
                              ) : (
                                <>
                                  Última actualización hace {secondsSinceUpdate}
                                  s
                                </>
                              )}
                            </p>
                          ...
                          <Card>
                            <CardContent className="p-3">
                              <div className="flex items-center gap-2 text-destructive mb-2 text-sm">
                                ...
                                <span className="font-medium">
                                  {job.errors.length} avisos durante el proceso
                                </span>
                              </div>
                ...
                {(isDone || isFailed) && job && (
                  <div className="space-y-3">
                    <div
                      className={`flex items-center gap-2 ${isDone ? "text-success" : "text-destructive"}`}
                    >
                      {isDone ? (
                    ...
                    {result && (
                      <Card>
                        <CardContent className="p-4 grid grid-cols-2 gap-2 text-sm">
                          <div>
                            Cuentas: <strong>{result.accountsCreated}</strong>
                          </div>
                          <div>
                            Períodos: <strong>{result.periodsCreated}</strong>
                          </div>
                          <div>
                            Compras: <strong>{result.purchasesCreated}</strong>
                          </div>
                          <div>
                            Ventas: <strong>{result.salesCreated}</strong>
                          </div>
                          <div>
                            Partidas creadas:{" "}
                            <strong>{result.journalEntriesCreated}</strong>
                          </div>
                          <div>
                            Publicadas:{" "}
                            <strong>{result.journalEntriesPosted}</strong>
                          </div>
                          <div>
                            Borrador:{" "}
                            <strong>{result.journalEntriesAsDraft}</strong>
                          </div>
                          <div>
                            Grupos activos:{" "}
                            <strong>{result.assetCategoriesCreated}</strong>
                          </div>
                          <div>
                            Activos fijos:{" "}
                            <strong>{result.fixedAssetsCreated}</strong>
                          </div>
                        </CardContent>
                      </Card>
                    )}
                      ...
                      <Card>
                        <CardContent className="p-4">
                          <div className="flex items-center gap-2 text-destructive mb-2">
                            <AlertCircle className="h-4 w-4" />
                            <span className="font-medium">
                              {job.errors.length} avisos/errores
                            </span>
                          </div>
                          <ScrollArea className="h-32">
                            <ul className="text-xs space-y-1">
                    ...
                    {isClearJob && result && (
                      <Card>
                        <CardContent className="p-4 space-y-3 text-sm">
                          <div>
                            Total borrado:{" "}
                            <strong>{result.deletedTotal ?? 0}</strong>
                          </div>
                          <ScrollArea className="h-60 rounded border">
                            <div className="space-y-3 p-3">
                              {result.deletedByStep &&
                                Object.keys(result.deletedByStep).length >
                                  0 && (
                                  <div className="space-y-1">
                                    {Object.entries(result.deletedByStep).map(
                                      ([key, value]) => (
                                        <div
                                          key={key}
                              ...
                              {result.verifiedEmptyByStep &&
                                Object.keys(result.verifiedEmptyByStep).length >
                                  0 && (
                              ...
                              {result.tableStats &&
                                Object.keys(result.tableStats).length > 0 && (
                                  <div className="space-y-1 border-t pt-3">
                                    {Object.entries(result.tableStats).map(
                                      ([key, value]) => (
                                        <div
                                          key={key}
                                          className="flex justify-between gap-3"
      ...
      <AlertDialog open={precheckOpen} onOpenChange={setPrecheckOpen}>
        <AlertDialogContent className="max-w-2xl">
          <AlertDialogHeader>
            <AlertDialogTitle>
              Hay datos existentes en la empresa
            </AlertDialogTitle>
            <AlertDialogDescription>
              Antes de importar, elige qué hacer con cada bloque que ya tiene
              registros. Si lo saltas, esa tabla no se tocará y se continuará
              con las demás.
            </AlertDialogDescription>
          </AlertDialogHeader>
                ...
                <CardContent className="p-4 space-y-3">
                  <div className="flex items-center justify-between gap-3 text-sm">
                    <div>
                      ...
                      <div className="text-muted-foreground">
                        Existentes: {item.existing} · A importar:{" "}
                        {item.incoming}
                      </div>
                    </div>
                  </div>
                  <RadioGroup
                    ...
                    onValueChange={(value) =>
                      setTableDecisions((prev) => ({
                        ...prev,
                        [item.key]: value as TableDecisionMode,
                      }))
                    }
                    className="gap-3"
                  >
                    <div className="flex items-start gap-3 rounded border p-3">
                      <RadioGroupItem
                        value="clear_then_import"
                        id={`clear-${item.key}`}
                      />
                      <Label
                        htmlFor={`clear-${item.key}`}
                        className="space-y-1 cursor-pointer"
                      >
                        <div>Borrar primero y luego importar</div>
                        <div className="text-xs text-muted-foreground">
                          Úsalo cuando quieras reemplazar completamente esta
                          tabla.
                        </div>
                      </Label>
                    </div>
                    <div className="flex items-start gap-3 rounded border p-3">
                      ...
                      <Label
                        htmlFor={`skip-${item.key}`}
                        className="space-y-1 cursor-pointer"
                      >
                        <div>Saltar esta tabla</div>
                        <div className="text-xs text-muted-foreground">
                          No se borrará ni se importará este bloque; se seguirá
                          con los demás.
                        </div>
                      </Label>
                    </div>
