...
export function PurchaseLinkManager({
  ...
  }, [enterpriseId, journalEntryId, bankReference, bankAccountId, linkedPurchases, updatePendingFlag, autoApply, toast]);
      ...
      for (let i = 0; i < purchases.length; i++) {
        ...
        try {
          const [{ error: linkError }, { error: legacyError }] = await Promise.all([
            ...
            supabase
              .from("tab_purchase_ledger")
              .update({ journal_entry_id: null, batch_reference: null, bank_account_id: null })
              .eq("enterprise_id", enterpriseId)
              .eq("id", p.id)
              .eq("journal_entry_id", journalEntryId)
              .is("deleted_at", null),
