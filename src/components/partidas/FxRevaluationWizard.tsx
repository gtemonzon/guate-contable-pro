...
import { Loader2, RefreshCw, AlertCircle, TrendingUp, TrendingDown, Sparkles, Info } from "lucide-react";
...
export function FxRevaluationWizard({ open, onOpenChange, enterpriseId, defaultYear, defaultMonth, onPosted }: Props) {
  ...
  const { loading, posting, buildPreview, postRevaluation } = useFxRevaluation();
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
        ...
        <DialogFooter>
          <Button variant="outline" onClick={() => onOpenChange(false)} disabled={posting}>Cancelar</Button>
