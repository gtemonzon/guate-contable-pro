...
BEGIN
  ...
  IF p_block IN ('journal_entries', 'accounts_periods', 'all') THEN
    ...
    RETURN QUERY SELECT 'journal_entries', 'tab_journal_entries', COALESCE(v_before - v_after, v_deleted_total), v_after;
  END IF;
  ...
  IF p_block IN ('fixed_assets', 'asset_categories', 'accounts_periods', 'all') THEN
    ...
    RETURN QUERY SELECT 'fixed_assets', 'fixed_assets', COALESCE(v_before - v_after, v_deleted_total), v_after;
    ...
    RETURN QUERY SELECT 'asset_categories', 'fixed_asset_categories', COALESCE(v_before - v_after, v_deleted_total), v_after;
  END IF;
  ...
  IF p_block IN ('purchases', 'accounts_periods', 'all') THEN
    ...
    RETURN QUERY SELECT 'purchases', 'tab_purchase_ledger', COALESCE(v_before - v_after, v_deleted_total), v_after;
    ...
    RETURN QUERY SELECT 'purchase_books', 'tab_purchase_books', COALESCE(v_before - v_after, v_deleted_total), v_after;
  END IF;
  ...
  IF p_block IN ('sales', 'accounts_periods', 'all') THEN
    ...
    RETURN QUERY SELECT 'sales', 'tab_sales_ledger', COALESCE(v_before - v_after, v_deleted_total), v_after;
  END IF;
  ...
  IF p_block IN ('accounts_periods', 'all') THEN
    ...
    RETURN QUERY SELECT 'periods', 'tab_accounting_periods', COALESCE(v_before - v_after, v_deleted_total), v_after;
    ...
    RETURN QUERY SELECT 'accounts', 'tab_accounts', COALESCE(v_before - v_after, v_deleted_total), v_after;
  END IF;
...
REVOKE ALL ON FUNCTION public.clear_legacy_import_block(bigint, text) FROM PUBLIC;
