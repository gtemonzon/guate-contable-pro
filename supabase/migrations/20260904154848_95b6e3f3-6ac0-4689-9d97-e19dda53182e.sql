...
BEGIN
  IF auth.uid() IS NULL THEN
    RAISE EXCEPTION 'No autenticado' USING ERRCODE = 'P0004';
  END IF;

  IF NOT (public.is_super_admin(auth.uid()) OR public.user_is_linked_to_enterprise(auth.uid(), p_enterprise_id)) THEN
    RAISE EXCEPTION 'No tiene acceso a esta empresa' USING ERRCODE = 'P0004';
  END IF;
...
DROP POLICY IF EXISTS "auth read tenant modules" ON public.tab_tenant_modules;
CREATE POLICY "auth read tenant modules"
ON public.tab_tenant_modules
FOR SELECT
TO authenticated
USING (
