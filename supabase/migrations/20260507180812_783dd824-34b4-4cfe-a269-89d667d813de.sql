...
DECLARE
  ...
  v_lock_key bigint;
  v_tables text[] := ARRAY[
    -- children of journal_entries / purchase_ledger
    'tab_purchase_journal_links',
    'tab_journal_entry_history',
    'tab_journal_entry_metadata_changes',
    -- ledgers (children of journal_entries, accounts, periods, books)
    'tab_purchase_ledger',
    'tab_sales_ledger',
    'tab_purchase_books',
    -- inventory closings (children of journal_entries, periods)
...
BEGIN
  ...
  IF NOT (
    ...
    OR public.user_is_linked_to_enterprise(auth.uid(), p_enterprise_id)
  ) THEN
    RAISE EXCEPTION 'Permission denied for enterprise %', p_enterprise_id USING ERRCODE = '42501';
  END IF;

  -- Advisory lock per enterprise
  ...
  -- Bypass audit triggers / immutability for the duration of THIS transaction
  PERFORM set_config('app.import_mode', 'on', true);
  PERFORM set_config('app.allow_posted_metadata_update', 'true', true);

  v_start := clock_timestamp();

  -- Pre-step: NULL out FK references inside tab_enterprise_config to accounts
  ...
  -- Pre-step: clear self-references in journal_entries to avoid FK conflicts on bulk delete
  ...
  -- Pre-step: clear self-references in accounts (parent_account_id)
  UPDATE public.tab_accounts
     SET parent_account_id = NULL
   WHERE enterprise_id = p_enterprise_id;

  -- Pre-step: NULL fixed_assets.disposal_je_id (in case asset deletion is later than journal_entries)
  UPDATE public.fixed_assets
     SET disposal_je_id = NULL
   WHERE enterprise_id = p_enterprise_id;

  -- Loop deletion phases
  FOREACH v_tbl IN ARRAY v_tables LOOP
    ...
    IF NOT EXISTS (
      ...
      v_stats := v_stats || jsonb_build_object(
        'table', v_tbl, 'skipped', true, 'reason', 'table not found'
      );
      CONTINUE;
    END IF;
...
GRANT EXECUTE ON FUNCTION public.hard_reset_enterprise(bigint) TO authenticated;
