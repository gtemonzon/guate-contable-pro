...
-- ============================================================
-- Account Balance Inspector: server-side RPC
...
DECLARE
  ...
  v_bal_type   text;
BEGIN
  -- Access check: user must be linked to enterprise or super admin
  IF NOT (
  ...
  WHERE id = p_enterprise_id::bigint   -- wrong, fix below
  ...
  -- Return rows with running balance computed via window function
  RETURN QUERY
  WITH period_rows AS (
    SELECT
      d.id            AS detail_id,
...
CREATE INDEX IF NOT EXISTS idx_jed_account_date
  ON public.tab_journal_entry_details (account_id)
  WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_je_enterprise_date
  ON public.tab_journal_entries (enterprise_id, entry_date)
  WHERE deleted_at IS NULL;
