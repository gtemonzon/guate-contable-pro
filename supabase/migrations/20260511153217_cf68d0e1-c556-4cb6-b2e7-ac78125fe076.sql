CREATE OR REPLACE FUNCTION public.get_trial_balance(p_enterprise_id bigint, p_start_date date, p_end_date date)
 RETURNS TABLE(account_id bigint, account_code text, account_name text, account_type text, balance_type text, level integer, parent_account_id bigint, opening_debit numeric, opening_credit numeric, period_debit numeric, period_credit numeric, closing_debit numeric, closing_credit numeric, opening_balance numeric, closing_balance numeric)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
  WITH RECURSIVE _access AS (
    SELECT 1
  ...
  _tree AS (
    ...
    FROM   _tree t
    JOIN   _accts c ON c.parent_account_id = t.descendant_id
  ),
  _opening AS (
    SELECT d.account_id,
  ...
  _agg AS (
    ...
    FROM   _tree t
    LEFT   JOIN _opening o ON o.account_id = t.descendant_id
    LEFT   JOIN _period  p ON p.account_id = t.descendant_id
